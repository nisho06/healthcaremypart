...
import 'package:healthcaresupportsystem/Pages/Diab_pages/facts_message.dart';
import 'package:healthcaresupportsystem/Pages/Diab_pages/Style.dart';
import 'package:flutter_dialogflow/dialogflow_v2.dart';
import 'package:provider/provider.dart';


class FlutterFactsChatBotdiab extends StatefulWidget {
  final BaseAuth auth=Auth();
  FlutterFactsChatBotdiab({Key key, this.title}) : super(key: key);
  final String title;
  @override
  _FlutterFactsChatBotdiabState createState() => new _FlutterFactsChatBotdiabState();
}

class _FlutterFactsChatBotdiabState extends State<FlutterFactsChatBotdiab> {
    ...
    return StreamProvider<DocumentSnapshot>.value(
      ...
      child: Scaffold(
        ...
        itemBuilder: (BuildContext context) {
          ...
          centerTitle: true,
          title: Text("Diabetes-specialist", style: Style.headerTextStyle,),
...
class DiabMessagePageBody extends StatefulWidget {
  final String uid;
  final BaseAuth auth;

  DiabMessagePageBody({this.uid, this.auth});
  @override
  _DiabMessagePageBodyState createState() => _DiabMessagePageBodyState();
}

class _DiabMessagePageBodyState extends State<DiabMessagePageBody> {
  ...
  void agentResponse(query,userData) async {
    ...
    await AuthGoogle(fileJson: "assets/Diab_images/doctor-of-diabetes-imxiuc-0e5e8721a4b5.json").build();
    ...
    }else if(response.queryResult.fulfillmentMessages[0]['text']['text'][0].toString().substring(0,8) =="You have" && response.queryResult.fulfillmentMessages[0]['text']['text'][0].toString().split('%')[1].trim().split('.')[0]=='possibility of having Diabetes'){
      widget.auth.setDiabPrediction(widget.uid,  double.parse(response.queryResult.fulfillmentMessages[0]['text']['text'][0].toString().split('%')[0].split(' ')[2]));
    }
  }
    ...
    return Row(
      ...
      children: <Widget>[
        FlatButton.icon(
            ...
            onPressed:(){
              _submitQuery('I need to get to know the probability of getting Diabetes',userData);
            },
            label: Text('Diabetes Predictions')),
        SizedBox(width: 8,),
        FlatButton.icon(
            ...
            shape: RoundedRectangleBorder(
                borderRadius: BorderRadius.circular(18)),
            color: Colors.blue[500],
            onPressed:()=>_submitQuery('I want to know about Diabetes.',userData),
